...
export default function ManageFindingIndex() {
  ...
  if (isDescEditable) {
    descAction = <button className="flex justify-center text-xs items-center bg-indigo-600 font-semibold border border-indigo-400 rounded-sm px-3 text-white" onClick={toggleDescEditable}>Save</button>
  } else {
    descAction = <button className="flex justify-center text-xs items-center bg-white font-semibold border border-gray-400 rounded-sm px-3" onClick={toggleDescEditable}>Edit</button>
  }
  ...
  return (
    <>
        ...
        <textarea disabled={!isDescEditable} onInput={(event) => {
          event.currentTarget.style.height = ""
          event.currentTarget.style.height = event.currentTarget.scrollHeight + "px"
        }} placeholder='Description' className='placeholder:text-gray-300 flex overflow-hidden border border-gray-300 rounded-sm resize-none h-24 min-h-24 w-[100%]'/>
