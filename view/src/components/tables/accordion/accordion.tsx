...
const Accordion = ({ accordionTitle, headers, content, isEditable, index, setContent }: AccordionData) => {
  const {
    register,
    formState: { errors },
    handleSubmit,
  ...
  function handleAddNewRow() {
    ...
    const isValidInsert = !newContent[index].details.length || !newContent[index].details.filter((detail: any) => detail.isEdited).length
  ...
  function insertNewDataToRow(data: any) {
    const newContent = content;
    (newContent[index].details as object[])[newContent[index].details.length - 1] = {
      ...data
    };
    setContent([...newContent]);
    reset()
  ...
  return (
    <div className={`relative overflow-hidden`}>
      ...
      <form className="flex flex-col overflow-hidden transition-all duration-500 bg-white max-h-0 peer-checked:max-h-fit" onSubmit={handleSubmit(insertNewDataToRow)}>
        {(content[index].details as any).map((contentRow: any, contentIndex: number) => {
          return (
            <div
              key={contentIndex}
              className={
        ...
        {isEditable && (
          <div className="flex justify-center p-3 underline cursor-pointer" onClick={handleAddNewRow}>
            Add New Row
          </div>
        )}
      </form>
