...
export default function CreateProjectForm() {
  ...
  function removeCurrentUser(user: any){
    const tempSelectedUser = selectedUsers.filter(selectedUser => selectedUser.id !== user.id);
    resetAssignedUserState(tempSelectedUser);
  }

  function resetAssignedUserState(tempSelectedUser: User[]){
    setSelectedUsers(tempSelectedUser);
    setAvailableUsers(users.filter(user => !tempSelectedUser.find(selectedUser => selectedUser.id === user.id)));
  }
          ...
          <div className="space-y-6 sm:space-y-5">
            ...
            <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
              ...
              <div className="mt-1 sm:mt-0 sm:col-span-2 flex flex-col gap-2">
                ...
                <span className='text-gray-500 hover:text-gray-700 cursor-pointer underline'>Create a new standard</span>
              </div>
            </div>
            ...
            <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
              ...
              <div className="mt-1 sm:mt-0 sm:col-span-2 flex flex-col gap-4">
                <div className="block max-w-lg w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                  <SelectBox items={availableUsers} onClickFunction={ getCurrentUser }/>
                </div>
                <div className="block max-w-lg w-full sm:text-sm border-gray-300 rounded-md flex flex-col gap-2">
                  {
                    selectedUsers.map((selectedUser, index) => <UserCard key={ index } user={ selectedUser } onClickFunction={ removeCurrentUser }/>)  
                  }
                </div>
              </div>
            </div>
