...
const CreateChecklistForm = () => {
  ...
  async function handleSubmitChecklistForm(data: ChecklistFormData) {
    await toast.promise(CreateChecklistHandler.handleCreateChecklistFormSubmit(data, JSON.stringify(sections)), {
  ...
  function deleteSection(){
    ...
    if (!search) return;
    const newSection = sections.filter(section => !section.name.toLowerCase().includes(search.toLowerCase()));
    setSections(newSection);
  }

  return (
    <>
      <form className="space-y-8" onSubmit={handleSubmit(watchIsSubmit ? handleSubmitChecklistForm : handleCreateSectionForm)}>
        <div className="space-y-6 sm:space-y-5">
          <PrimaryButton content="Save" type='submit' onClick={() => setValue('isSubmit', true)}></PrimaryButton>
            ...
            <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:pt-5">
              <InputText
                ...
                register={register('section', {
                  required: !watchIsSubmit ? 'Section Name is required.' : false,
                  validate: (section) => {
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-4">
              ...
              />
              <button type='button' onClick={deleteSection} className="inline-flex items-center justify-center h-8 px-4 text-xs font-medium leading-4 text-white bg-blue-600 border border-transparent rounded shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                Delete
              </button>
            </div>
